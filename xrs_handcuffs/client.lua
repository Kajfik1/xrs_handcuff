        ...
        },
        {
            ...
            canInteract = function(entity)
                if IsPedAPlayer(entity) then
                    ...
                    if ESX.PlayerData.job.name == "police" and target and playerState then
                        return true
                    else
                        return false
                    end
...
RegisterNetEvent("xrs_handcuff:cuffMe", function(playerheading, playercoords, playerlocation, rope)
    ...
    if exports["fc-radialmenu"]:isRadialmenuIsShowed() then
        exports["fc-radialmenu"]:closeRadial(false)
    end
    if rope then
        timer = 900
    end
